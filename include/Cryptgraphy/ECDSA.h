...
struct secpTraits {
	...
	using affinpoint_t = ECAffinPoint<modint_t>;
	using project_t = ECProject<modint_t>;
};

template<class secpType>
struct secpParameters {};
...
struct secpParameters<secp112r1> : public secpTraits<bigint<2>> {
	...
	static inline const affinpoint_t::Factory afinpoint = ECParam;
	static inline const project_t::Factory project = ECParam;
	static inline const affinpoint_t G = afinpoint(
...
public:
	...
	using typename secpParameters<Param>::baseint_t;
	using typename secpParameters<Param>::int_t;
	using typename secpParameters<Param>::modint_t;
	using typename secpParameters<Param>::affinpoint_t;
	using typename secpParameters<Param>::project_t;

	static inline RandomGenerator rd{};

	using message_t = Cryptgraphy::cbytearray<32>;
